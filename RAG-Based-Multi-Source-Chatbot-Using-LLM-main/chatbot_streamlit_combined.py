...
token=os.getenv("API_KEY")
token='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'
...
def display_chatbot_page():
    ...
    with st.expander("Initialize the LLM Model"):
        ...
        with st.form("setting"):
            row_1 = st.columns(3)
            with row_1[0]:
                text = st.text_input("Hugging Face Token (No need to insert)", type='password',value= token)

            with row_1[1]:
                llm_model = st.text_input("LLM model", value="lmstudio-community/Meta-Llama-3.1-8B-Instruct-GGUF")   #tiiuae/falcon-7b-instruct

            with row_1[2]:
                instruct_embeddings = st.text_input("Instruct Embeddings", value="intfloat/multilingual-e5-large")
    ...
    with st.form("document_input"):
        ...
        with row_1[0]:
            instruct_embeddings = st.text_input(
                "Model Name of the Instruct Embeddings", value="intfloat/multilingual-e5-large"
            )
